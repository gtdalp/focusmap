    ...
    Focusmap.prototype = {
        ...
        createTemplate: function () {
        	...
        	for (; i < length; i++) {
        		...
        		href = dataF.href || '';
        		// 图片
        		html += '<li style="background-image:url(' + src + ');' + (i === 0 ? 'opacity:1;z-index:2;' : '') + '">' + 
        					(href ? '<a href="' + href + '"></a>' : '') + 
        				'</li>';
        ...
        // 事件
        event: function () {
        	var op = this.options, Animate, index = 0,
        		time = op.time,
        		self = this,
        	...
        	// 执行动画
        	ele.hover( function(){    // 鼠标放上去图片的时候清除动画
				clearInterval( Animate );
			}, function(){     // 鼠标移开图片的时候重新执行动画
        ...
        toggleImg: function (index) {
        	...
        	// 切换图片
			img.eq(index).animate({"opacity" : 1, 'z-index': 2}, 1000).siblings()
